...
public:
	...
	bool getNext(float& x1, float& y1, float& x2, float& y2, int& nbr1, int& nbr2)
	{
		if(iteratorEdges == 0)
			return false;
		...
		nbr1 = iteratorEdges->sitenbr1;
		nbr2 = iteratorEdges->sitenbr2;
...
private:
	...
	void pushGraphEdge(float x1, float y1, float x2, float y2, int, int);

	void VoronoiDiagramGenerator::openpl();
	void VoronoiDiagramGenerator::line(float x1, float y1, float x2, float y2, int, int);
